 ...
 *
 * Translation files live in the "SnowFixer_translations" folder next to the executable, one file per language
 * named by its IETF/ISO language code (e.g. "en.json", "fr.json"). The schema is i18next-style nested JSON: nested
 * objects whose leaf string values are addressed by dot-separated keys, e.g. {"launcher": {"title": "..."}} is
 * looked up as "launcher.title". A reserved top-level "_language" key holds the language's native display name
 ...
 *
 * Mirrors AutoBlend's own ABLocale (same file layout, same lookup semantics) - a straight port under this
 * project's own name, not a from-scratch design.
 */
class SFLocale {
public:
    struct Language {
...
/// @brief Shorthand for SFLocale::tr.
[[nodiscard]] inline auto SFTr(const std::string& key, const char* defaultValue) -> wxString
{
    return SFLocale::tr(key, defaultValue);
}
